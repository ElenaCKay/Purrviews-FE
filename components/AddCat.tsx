...
const AddCat = ({ setAddCat, addCat, setApiUserInfo, apiUserInfo }) => {
    ...
    return (
        ...
        >
            {err && <Text tw="w-full bg-red-700 text-xl text-white top-0 absolute">{err}</Text>}
            <Text style={{ fontFamily: "Pacifico-Regular" }} tw="text-5xl pt-7 text-purple-900 underline">
                Add A Cat
            </Text>
            <TouchableOpacity tw=" bg-gray-300 rounded-3xl mb-12" style={{ elevation: 6 }} onPress={pickCatImage}>
                <Image tw="h-44 w-40 items-center" resizeMode="contain" source={{ uri: currentCat.cat_img }} />
            </TouchableOpacity>
            <Input
